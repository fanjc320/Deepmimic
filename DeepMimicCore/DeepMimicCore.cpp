...
{
	mScene->Update(timestep);//注释画面静止 mScen:cDrawSceneImitate
}
...
void cDeepMimicCore::Draw()
{
	if (EnableDraw())
	{
		mDefaultFrameBuffer->BindBuffer();//去掉后屏幕在白和灰之间闪烁
		mScene->Draw();//注释就黑屏,cDrawSceneImitate
...
void cDeepMimicCore::SetupScene()
{
	...
	if (EnableDraw())
	{
		cSceneBuilder::BuildDrawScene(scene_name, mScene);//走这里
	}
	else
	{
